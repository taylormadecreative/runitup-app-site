...
// Social sign-in uses the native plugin (ID-token flow) — only offer it in the
// iOS app. On the web build the buttons would dead-end (the Supabase Google
// provider is configured with the iOS OAuth client, which rejects web
// redirects), so they aren't rendered until web OAuth is configured.
